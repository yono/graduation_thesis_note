...
from graduate.note.models import Tag
from django.forms.extras.widgets import SelectDateWidget
from django.forms.widgets import Widget, Select, TextInput, MultiWidget
from django.utils.safestring import mark_safe
from django.template import defaultfilters
from django import forms

__all__ = ('SelectTimeWidget', 'SplitSelectDateTimeWidget', 'TagWidget')
...
class SelectTimeWidget(Widget):
    ...
    def __init__(self, attrs=None, hour_step=None, minute_step=None, second_step=None, twelve_hr=False, use_seconds=False):
        """
    ...
    def render(self, name, value, attrs=None):
        ...
        # select tag と値がずれてる場合修正する
        if self.hour_step and (hour_val % self.hour_step) != 0:
            hour_val = self.adjast_val_to_step(hour_val, self.hours)

        if self.minute_step and (minute_val % self.minute_step) != 0:
            minute_val = self.adjast_val_to_step(minute_val, self.minutes)

        if self.second_step and (second_val % self.second_step) != 0:
            second_val = self.adjast_val_to_step(second_val, self.seconds)
    ...
    def value_from_datadict(self, data, files, name):
        ...
        h = int(data.get(self.hour_field % name, 0)) # hour
        m = int(data.get(self.minute_field % name, 0)) # minute 
        s = int(data.get(self.second_field % name, 0)) # second
    ...
    def adjast_val_to_step(self, val, val_list):
        smallest = 1000
        select_index = 0
        for i,v in enumerate(val_list):
            if fabs(val - v) < smallest:
                smallest = fabs(val - v)
                select_index = i
        return val_list[select_index]
...
class SplitSelectDateTimeWidget(MultiWidget):
    ...
    def value_from_datadict(self, data, files, name):
        ...
        return datetime(*strptime(values[0]+values[1], "%Y-%m-%d%H:%M:%S")[0:6])

class ElapsedTimeWidget(MultiWidget):
    ...
    def __init__(self, attrs=None):
        ...
        super(ElapsedTimeWidget, self).__init__(widgets, attrs)
    ...
    def format_output(self, rendered_widgets):
        rendered_widgets.insert(-1, u'時間')
        rendered_widgets.append(u'分')
...
class TagWidget(forms.TextInput):
    ...
    def render(self, name, value, attrs=None):
        if isinstance(value, (list, tuple)):
            ...
            for key in value:
                try:
                    #cat = Tag.objects.get(name=key)
                    cat = Tag.objects.get(pk=key)
                    new_value.append(cat.name)
                except:
                    pass
    ...
    def value_from_datadict(self, data, files, name):
        tags = data.get(name, 0)
        tag_list = tags.split(',')
        tag_ids = []
        if len(tag_list) > 0:
            print tag_list
            for tag in tag_list:
                atag = tag.rstrip(' ').lstrip(' ')
                try:
                    tag_obj = Tag.objects.get(name=atag)
                    tag_ids.append(str(tag_obj.id).decode())
                except:
...
class TagField(forms.Field):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(initial, (list, tuple)):
            for init in initial:
                tag = Tag.objects.filter(name=init)
                if tag:
                    new_initial.append(tag[0].name)
        super(TagField, self).__init__(initial=",".join(new_initial), *args, **kwargs)
